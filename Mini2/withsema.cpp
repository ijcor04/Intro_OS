...
//The addition of values into TOTAL will also be regulated/protected by a semphore.
...
void* add_to_total(void* thread_id) 
{
	...
	{
        sem_wait(&semaphore);   // Lock access to TOTAL
        TOTAL =(TOTAL + array[i]);
        sem_post(&semaphore);   // Unlock access to TOTAL
    }
...
int main() 
{
    ...
    {
        pthread_create(&threads[i], nullptr, add_to_total, (void*)&thread_ids[i]);
    }
    ...
    // Print TOTAL
    std::cout << "TOTAL with semaphores: " << TOTAL << std::endl;
